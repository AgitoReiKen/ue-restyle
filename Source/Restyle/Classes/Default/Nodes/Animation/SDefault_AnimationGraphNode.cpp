...
void SDefault_AnimationGraphNode::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	SDefault_GraphNodeRestyleK2Base::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
...
TSharedRef<SWidget> SDefault_AnimationGraphNode::CreateTitleWidget(TSharedPtr<SNodeTitle> InNodeTitle)
{
	...
	AnimGraphNode->OnNodeTitleChangedEvent().AddSP(this, &SDefault_AnimationGraphNode::HandleNodeTitleChanged);

	return SDefault_GraphNodeRestyleK2Base::CreateTitleWidget(InNodeTitle);
}

void SDefault_AnimationGraphNode::HandleNodeTitleChanged()
{
...
void SDefault_AnimationGraphNode::GetNodeInfoPopups(FNodeInfoContext* Context, TArray<FGraphInformationPopupInfo>& Popups) const
{
	SDefault_GraphNodeRestyleK2Base::GetNodeInfoPopups(Context, Popups);
...
{
	CachedContentArea = SDefault_GraphNodeRestyleK2Base::CreateNodeContentArea();
...
TSharedRef<SWidget> SDefault_AnimationGraphNode::CreateNodeFunctionsWidget(UAnimGraphNode_Base* InAnimNode, TAttribute<bool> InUseLowDetail)
{
	return SNew(SAnimNodeFunctionsWidget, InAnimNode)
		.UseLowDetail(InUseLowDetail);
}

ACCESS_PRIVATE_FIELD(UAnimGraphNode_Base, FName, Tag);

TSharedRef<SWidget> SDefault_AnimationGraphNode::CreateNodeTagWidget(UAnimGraphNode_Base* InAnimNode, TAttribute<bool> InUseLowDetail)
{
	auto Tag = access_private::Tag(*InAnimNode);
	return SNew(SLevelOfDetailBranchNode)
		.Visibility_Lambda([Tag]() { return (Tag != NAME_None) ? EVisibility::Visible : EVisibility::Collapsed; })
...
void SDefault_AnimationGraphNode::ReconfigurePinWidgetsForPropertyBindings(UAnimGraphNode_Base* InAnimGraphNode, TSharedRef<SGraphNode> InGraphNodeWidget, TFunctionRef<TSharedPtr<SGraphPin>(UEdGraphPin*)> InFindWidgetForPin)
{
...
#define LOCTEXT_NAMESPACE "SDefault_PoseWatchOverlay"
